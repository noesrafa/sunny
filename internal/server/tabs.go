...
func (s *server) openTab(w http.ResponseWriter, r *http.Request) {
	...
	a, ok := s.store.Agent(req.AgentID)
	if !ok {
		http.NotFound(w, r)
		return
	}
	...
	if convID == "" {
		...
		s.publish(evts.ConvCreated, req.AgentID, convID)
	} else {
		// Verify the conv exists; refuse to open a tab pointing
		// at a non-existent journal.
		meta, _, err := s.conv.Get(req.AgentID, convID)
		if err != nil {
			if errors.Is(err, conversation.ErrNotFound) {
				http.NotFound(w, r)
				return
...
// ~/.sunny/agents/<id>/conversations/<old_conv_id>/ — only the tab's
// pointer changes.
//
...
func (s *server) rebindTabConv(w http.ResponseWriter, r *http.Request) {
	...
	a, ok := s.store.Agent(existing.AgentID)
	if !ok {
		http.NotFound(w, r)
		return
	}
	...
	s.publish(evts.ConvCreated, existing.AgentID, meta.ID)
...
// carry the tab id, the agent id, and the conv id so subscribers
