...
type Engine struct {
	...
	// envFn samples the daemon's runtime environment for inclusion
	// in the system prompt. Optional — nil means no env block.
	// Invoked once per turn so date and IPs stay fresh.
	envFn func() *prompt.Env
}
...
// WithEnvFn returns the engine with envFn set. Daemon side wires
// this in at construction; tests leave it nil to skip the env block.
func (e *Engine) WithEnvFn(fn func() *prompt.Env) *Engine {
	e.envFn = fn
	return e
}
...
func (e *Engine) Turn(ctx context.Context, agent *store.Agent, messages []provider.Message, opts TurnOptions) (<-chan provider.Event, error) {
	...
	system, err := prompt.Build(agent, env)
	if err != nil {
