...
// Stream is an open SSE response. Next blocks until the daemon emits
// one event, the context is cancelled, or the stream ends. On a
// terminal event (Done or Error) Next returns ok=true with that
// event; the next call returns ok=false.
type Stream struct {
	ctx    context.Context
...
// Turn opens POST /agents/{slug}/conversations/{convID}/turn and
// returns the live SSE stream. Cancellation: pass a cancellable ctx;
// calling Stream.Close also cancels.
...
// Next blocks until the next SSE event arrives, the context is
// cancelled, or the stream ends. Returns (event, true, nil) on a
// successful read, (nil, false, nil) on clean end-of-stream, and
...
func (s *Stream) Next() (Event, bool, error) {
	...
	// Clean EOF — daemon closed the connection without a Done event.
